...
Single stack: VPC, ECS cluster, RDS Postgres (t3.micro, PostgreSQL 15), two Fargate task definitions. The Go service is created via `ApplicationLoadBalancedFargateService` (owns the ALB); the astro service is a plain `FargateService` attached to the same ALB via a second listener on port 8081. DB credentials are injected as ECS secrets from Secrets Manager into the Go container's env vars; the astro container doesn't need DB access. Health checks: `/health` on port 80 for backend, port 8000 for astrobackend (note: astrobackend runs on 8000 inside the container, 8081 externally).

Both `backend/repository.json` and `astrobackend/repository.json` must exist (created by `make aws-push` or `make -C {backend,astrobackend} create-repository`) before running `cdk synth` — the stack reads them to resolve the ECR repository ARNs.
...
- Environment variables the code reads: `OBS_DB_HOST`, `OBS_DB_USER`, `OBS_DB_PASSWORD`, `OBS_DB_NAME`, `OBS_DB_PORT` (Go server, via compose); `OBS_SERVER_PORT` (optional override, defaults to 80).
